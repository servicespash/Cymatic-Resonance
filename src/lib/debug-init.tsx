...
export function initDebugOverlay() {
  ...
  function DebugOverlay() {
    ...
    return (
      <div>
        <button onClick={() => setErrors([])} style={{marginBottom: '5px', fontSize: '10px'}}>Clear</button>
        {errors.map((e, i) => <div key={i} style={{marginBottom: '5px', borderBottom: '1px solid #444'}}>{e}</div>)}
      </div>
    );
  }
