...
export function useCall(opts: {
  ...
  const connectionTimers = useRef<Record<string, NodeJS.Timeout>>({});
    ...
    (remoteId: string): RTCPeerConnection => {
      ...
      const pc = createPeer({
        ...
        onConnectionStateChange: (state) => {
          setRemote(remoteId, { state });
          if (state === "connected") {
            if (connectionTimers.current[remoteId]) {
              clearTimeout(connectionTimers.current[remoteId]);
              delete connectionTimers.current[remoteId];
            }
          }
        },
      });

      // 10s Connection timeout fallback & ICE restart
      connectionTimers.current[remoteId] = setTimeout(() => {
        if (pc.connectionState !== "connected") {
          toast.info("Reconnecting call (network relay)...");
          try {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const sig = joinCallChannel(callId, selfId, async (p) => {
          ...
          if (p.type === "bye") {
            ...
            if (connectionTimers.current[p.from]) {
              clearTimeout(connectionTimers.current[p.from]);
              delete connectionTimers.current[p.from];
            }
            setRemotes((r) => {
              const n = { ...r };
              delete n[p.from];
    ...
    return () => {
      ...
      peersRef.current = {};
      Object.values(connectionTimers.current).forEach(clearTimeout);
