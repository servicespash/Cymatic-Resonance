  ...
  return (
    <div className="flex flex-col h-full bg-black/5 rounded-xl border border-white/5 p-4">
      ...
      <div className="flex items-center gap-2 border-t border-white/10 pt-3">
        <button className="text-muted-foreground hover:text-white" title="Attach file"><Paperclip className="size-5" /></button>
        <button className="text-muted-foreground hover:text-white" title="Record audio"><Mic className="size-5" /></button>
        <input 
          value={body}
        ...
        />
        <button onClick={sendMessage} className="bg-frequency text-white p-2 rounded-lg"><Send className="size-5" /></button>
      </div>
    </div>
  );
};
