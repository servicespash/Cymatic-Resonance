...
const nav: {
  ...
  { to: "/directory", label: "Team Directory", icon: Users, adminOnly: true },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (profile?.org_id) {
      supabase
        .from("organizations")
        .select("id, name, access_code, org_type")
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-white/5 glass px-4 py-3 md:px-6">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <button
              onClick={toggleTheme}
              className="flex size-8 items-center justify-center rounded-lg border border-white/10 hover:bg-white/5 transition-colors"
