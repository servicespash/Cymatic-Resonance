...
export const TaskWorkaroundPage = ({ taskId }: { taskId: string }) => {
  ...
  useEffect(() => {
    ...
    const taskChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setTask(newData);
          setResearch(newData.description || "");
        },
      )
      .subscribe();
    ...
    const activityChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          setActivity((prev) => [...prev, payload.new as Activity]);
        },
      )
      .subscribe();
  ...
  const advanceStatus = async () => {
    ...
    if (currentIndex < steps.length - 1) {
      ...
      try {
        ...
        // Rollback
        setTask(task);
        setActivity((prev) => prev.filter((a) => a.id !== "temp"));
  ...
  const saveNotes = async () => {
    await supabase.from("tasks").update({ description: research }).eq("id", taskId);
    ...
    <div className="p-6 space-y-8">
        ...
        <ul className="space-y-2">
          {activity.map((act) => (
            <li key={act.id} className="text-xs">
              {act.status} at {new Date(act.created_at).toLocaleString()}
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="space-y-4">
        ...
        <Textarea
          ...
          placeholder="Do your research and notes here..."
          className="h-64"
        />
            ...
            <li key={att.id} className="flex items-center justify-between p-2 bg-secondary rounded">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      supabase.storage.from("task-attachments").getPublicUrl(att.file_url).data
                        .publicUrl,
                    )
                  }
                >
