            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {TYPES.map((t) => (
                    <button
                      key={t}
                      onClick={() => setType(t)}
                      className={`rounded-md border px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest ${type === t ? "border-accent/40 bg-accent/10 text-accent" : "border-white/10 bg-white/5 text-muted-foreground hover:text-foreground"}`}
                    >
                      {t}
                    </button>
                  ))}
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-full justify-start bg-white/5 border-white/10 gap-2"
                    >
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Trip, appointment, etc."
        ...
        {list.map((l) => (
          <div key={l.id} className="flex items-center justify-between py-2.5">
            <div>
              ...
              {l.reason && (
                <div className="mt-0.5 text-xs text-muted-foreground line-clamp-1">{l.reason}</div>
              )}
            </div>
...
function StatusPill({ status }: { status: "pending" | "approved" | "denied" }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest ${map[status]}`}
    >
      {status}
    </span>
  );
