...
export function BrandPanel({ orgId, logoUrl, accentColor, onChange }: Props) {
  ...
  const upload = async (file: File) => {
    ...
    const { error } = await supabase.rpc("update_org_brand", { _logo_url: path, _accent_color: null } as any);
  ...
  const saveColor = async (c: string) => {
    setColor(c);
    const { error } = await supabase.rpc("update_org_brand", { _logo_url: null, _accent_color: c } as any);
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="grid size-24 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            {signedUrl ? (
              ...
              <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">No logo</span>
            )}
          </div>
          <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0])} />
          <button onClick={() => inputRef.current?.click()} disabled={busy} className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-xs hover:bg-white/10 disabled:opacity-50">
            <Upload className="size-3" /> {busy ? "Uploading…" : "Upload logo"}
          </button>
        </div>

        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Accent color</div>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {PRESETS.map((c) => (
              <button key={c} onClick={() => saveColor(c)} aria-label={c}
            ...
            <label className="ml-2 inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/5 px-2 py-1 text-xs">
              <input type="color" value={color} onChange={(e) => saveColor(e.target.value)} className="h-5 w-7 cursor-pointer bg-transparent" />
              <span className="font-mono">{color}</span>
            </label>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">Color reflects through highlights, buttons, and glow.</p>
        </div>
      </div>
    </section>
  );
