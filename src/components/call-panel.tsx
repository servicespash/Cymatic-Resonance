  ...
  return (
    <div className="glass-strong flex items-center justify-between rounded-xl border border-white/10 p-3">
      <div className="flex items-center gap-3">
        <div
          className={`size-3 rounded-full ${state.status === "connected" ? "bg-green-500 animate-pulse" : "bg-muted"}`}
        />
        <span className="text-sm font-medium">
          {state.status === "connected" ? "Call Active" : "No Active Call"}
        </span>
      ...
      {state.status === "idle" ? (
        ...
        >
          ...
          {state.status === "connecting" ? "Connecting..." : "Leave"}
        </button>
      )}
    </div>
  );
