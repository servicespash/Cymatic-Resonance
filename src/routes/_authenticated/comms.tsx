...
function CommsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const r of (rd ?? []) as { channel_id: string; last_read_at: string }[])
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: m } = await supabase
        ...
        .limit(200);
      setMsgs((m ?? []) as Msg[]);
      const ids = (m ?? []).map((x: Tables<"messages">) => x.id);
      if (ids.length) {
  ...
  useEffect(() => {
    ...
    let lastSent = 0;
    const ch = supabase.channel(`presence-${active.id}`, {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      if (data) {
        setActiveCall(data as Tables<"calls">);
        const { data: parts } = await supabase
          .from("call_participants")
          .select("user_id, state")
          .eq("call_id", data.id)
          .eq("state", "joined");
        setCallJoined(((parts ?? []) as Tables<"call_participants">[]).map((p) => p.user_id));
      } else {
        setActiveCall(null);
        setCallJoined([]);
      }
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "calls", filter: `channel_id=eq.${active.id}` },
        () => load(),
      )
  ...
  const sendMessage = async (text: string, files: File[], audio?: RecordedAudio) => {
    ...
    try {
      ...
      for (const f of files)
        uploads.push(
          uploadOne(f, f.name, f.type || "application/octet-stream", (msg as { id: string }).id),
        );
      if (audio)
        uploads.push(
          uploadOne(
  ...
  return (
    <div className="-m-4 md:-m-6 flex h-[calc(100vh-4.5rem)] flex-col lg:grid lg:grid-cols-[380px_1fr]">
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
                  ...
                  <DialogContent className="glass-strong border-white/10">
                    ...
                    <DialogFooter>
                      <button
                        ...
                        className="rounded-xl bg-frequency px-4 py-2 text-sm font-semibold text-primary-foreground resonance-glow"
                      >
                        Create
                      </button>
                    </DialogFooter>
              ...
              <Dialog open={newDmOpen} onOpenChange={setNewDmOpen}>
                <DialogTrigger asChild>
                  <button
                    className="grid size-10 place-items-center rounded-full bg-frequency text-primary-foreground resonance-glow transition hover:brightness-110"
                    aria-label="New chat"
                  >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center justify-between gap-2">
                    <p
                      className={`truncate text-sm ${u > 0 ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {preview}
                    </p>
                    {u > 0 && (
      ...
      <section
        className={`flex min-h-0 flex-col bg-background/40 ${active ? "flex" : "hidden lg:flex"}`}
      >
        {!active ? (
          ...
          <>
            <header className="flex items-center gap-3 border-b border-white/5 bg-card/50 px-4 py-3 backdrop-blur md:px-6">
              <button
                ...
                className="grid size-9 shrink-0 place-items-center rounded-full text-muted-foreground transition hover:bg-white/10 hover:text-foreground lg:hidden"
                aria-label="Back"
              >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                  ...
                  {active.kind === "dm"
                    ? (activeOther?.role ?? "online")
                    : `${Object.keys(senders).length} members`}
                </div>
              </div>
              <button
                onClick={() => startCall("audio")}
                className="grid size-10 place-items-center rounded-full text-muted-foreground transition hover:bg-white/10 hover:text-accent"
              ...
              <div className="flex items-center gap-3 border-b border-accent/20 bg-accent/10 px-4 py-3 md:px-6 animate-fade-up">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    {callJoined.length > 0
                      ? `${callJoined.length} in call · ${callJoined
                          .slice(0, 3)
                          .map((id) => senders[id]?.full_name?.split(" ")[0] ?? "…")
                          .join(", ")}${callJoined.length > 3 ? ` +${callJoined.length - 3}` : ""}`
                ...
                {msgs.map((m, i) => {
                  ...
                  const showHeader =
                    ...
                    (newDay ||
                      ...
                      timeDiffMin(prev.created_at, m.created_at) > 5);
                  const showAvatar =
                    !me && (i === msgs.length - 1 || msgs[i + 1].sender_id !== m.sender_id);
                  ...
                  return (
                    <div key={m.id} className="w-full">
                      ...
                      <div
                        className={`group flex items-end gap-3 w-full ${me ? "justify-end" : "justify-start"} ${showHeader ? "mt-5" : "mt-1"}`}
                      >
                        {!me && (
                        ...
                        <div
                          className={`flex flex-col ${me ? "items-end text-right" : "items-start text-left"} max-w-[85%] lg:max-w-[75%]`}
                        >
                          {showHeader && (
                            <div className="mb-1.5 flex items-center gap-1.5 px-1.5">
                              ...
                              {s?.role === "admin" && (
                                <span className="rounded-md bg-frequency/20 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-widest text-accent">
                                  Admin
                                </span>
                              )}
                            </div>
                          ...
                          <div className="group/bubble relative flex items-end gap-2 max-w-full">
                            <div
                              className={`flex flex-col gap-1.5 ${me ? "items-end" : "items-start"} max-w-full`}
                            >
                              {m.body && (
                                <div
                                  className={`relative px-6 py-4 text-[15px] leading-relaxed shadow-md glass ${
                                    me
                                ...
                                >
                                  ...
                                  <div
                                    className={`mt-1.5 flex items-center justify-end gap-1 ${me ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                                  >
                            ...
                            {pickerFor === m.id && (
                              <div
                                className={`absolute z-20 flex gap-1 rounded-full border border-white/10 bg-card/95 p-1.5 shadow-2xl backdrop-blur-xl ${me ? "right-8 -top-12" : "left-8 -top-12"}`}
                              >
                                {QUICK_EMOJIS.map((e) => (
                                  <button
                                    key={e}
                                    onClick={() => react(m.id, e)}
                                    className="rounded-full px-1.5 py-1 text-base hover:bg-white/10"
                                  >
                                    {e}
                                  </button>
                                ))}
                                  ...
                                  <button
                                    ...
                                    className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs transition ${
                                      mine
                                        ...
                                        : "border-white/10 bg-card/60 text-muted-foreground hover:bg-white/10"
                                    }`}
                                  >
                                    <span>{emoji}</span>
            ...
            {pending.length > 0 && (
              <div className="mx-auto flex w-full max-w-none px-4 pt-3 md:px-8 lg:px-12 flex-wrap gap-2 border-t border-white/5">
                {pending.map((f, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-2 rounded-lg bg-white/5 px-2.5 py-1.5 text-xs ring-1 ring-white/10"
                  >
            ...
            {(() => {
              ...
              const names = activeTypers.map(
                (id) => senders[id]?.full_name?.split(" ")[0] ?? "Someone",
              );
              const label =
                names.length === 1
                  ? `${names[0]} is typing`
            ...
            <form
              onSubmit={send}
              className="border-t border-white/5 bg-card/30 px-4 py-3 backdrop-blur md:px-8 lg:px-12"
            >
                ...
                {recording ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="grid size-11 shrink-0 place-items-center rounded-full text-muted-foreground transition hover:bg-white/10 hover:text-foreground"
                      aria-label="Attach"
                    >
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.value) typingSendRef.current?.(Date.now());
                        }}
                    ...
                    {body.trim() || pending.length > 0 ? (
                      <button
                        type="submit"
                        disabled={sending}
                        className="grid size-11 shrink-0 place-items-center rounded-full bg-frequency text-primary-foreground resonance-glow transition hover:brightness-110 disabled:opacity-40"
                        aria-label="Send"
                      >
                      ...
                      <button
                        ...
                        className="grid size-11 shrink-0 place-items-center rounded-full bg-frequency text-primary-foreground resonance-glow transition hover:brightness-110"
...
function formatListTime(d: Date) {
  ...
  if (sameDay(d.toISOString(), now.toISOString()))
