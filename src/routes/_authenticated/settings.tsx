...
function SettingsPage() {
  ...
  const saveOrg = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.rpc("update_org_settings", {
      _name: org.name, _org_type: org.org_type, _cutoff: org.day_start_cutoff, _tz: org.timezone,
    });
    setBusy(false);
    if (error) return toast.error(error.message);
    setOrg(data as Org);
        ...
        <section className="glass-strong rounded-2xl p-6 resonance-glow">
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {isAdmin && (
                <button onClick={rotate} className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-3 py-2 text-xs transition hover:bg-white/10">
          ...
          {isAdmin && (
            <form onSubmit={saveOrg} className="mt-5 grid gap-3 sm:grid-cols-2">
              <Field label="Workspace name" value={org.name} onChange={(v) => setOrg({ ...org, name: v })} />
              <Field label="Type" value={org.org_type} onChange={(v) => setOrg({ ...org, org_type: v })} />
              <Field label="Day-start cutoff" value={org.day_start_cutoff?.slice(0, 5) ?? "09:00"} onChange={(v) => setOrg({ ...org, day_start_cutoff: v })} />
                ...
                {m.id !== user?.id && (
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => setRole(m.id, m.role === "admin" ? "member" : "admin")}
                      ...
                      {m.role === "admin" ? <UserIcon className="size-3" /> : <Crown className="size-3" />}
      ...
      {isAdmin && org && (
        <section className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Deleting this workspace removes every channel, message, attendance record, and unlinks all members. This cannot be undone.
          </p>
          <AlertDialog>
            ...
            <AlertDialogContent className="glass-strong border-red-500/20">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Type the workspace name <span className="font-mono text-red-300">{org.name}</span> to confirm.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <input
                value={confirmName}
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={deleteOrg} className="bg-red-500/80 hover:bg-red-500">Delete forever</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </section>
