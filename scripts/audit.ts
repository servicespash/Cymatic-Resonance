...
if (!DB_URL) {
    console.error(`${COLORS.red}❌ ERROR: DATABASE_URL environment variable is missing.${COLORS.reset}`);
    process.exit(1);
}
...
async function runAudit() {
    ...
    try {
        console.log(`${COLORS.blue}🔄 Connecting to live database for deep system audit...${COLORS.reset}`);
        ...
        for (const table of tables) {
            ...
            const hasAllPolicies = ['SELECT', 'INSERT', 'UPDATE', 'DELETE'].every(cmd => commands.includes(cmd));
        ...
        console.log(realtimeTables.length > 0 ? realtimeTables.join(', ') : `${COLORS.yellow}⚠️ No tables in Realtime!${COLORS.reset}`);
        ...
        console.error(`${COLORS.red}❌ Database Audit Failed: ${err.message}${COLORS.reset}`);
        process.exit(1);
    }
}
